...
public class SupplyServiceImpl implements SupplyService {
    ...
    public SupplyResponseDTO suspend(
        ...
        if (
            ...
            throw new BadRequestException(
                    "Solo se pueden suspender suministros activos"
            );
        }
    ...
    @Override
    public SupplyResponseDTO reconnect(
            UUID id,
        ...
        if (
            ...
            throw new BadRequestException(
                    "Solo se pueden reconectar suministros cortados"
            );
        }

        supply.setStatus(SupplyStatus.RECONNECTED);
    ...
    @Override
    public SupplyResponseDTO cutOff(
            UUID id,
            SuspendSupplyDTO dto
    ) {
        ...
        if (
            ...
            throw new BadRequestException(
                    "Solo se pueden cortar suministros suspendidos"
            );
        }
    ...
    private SupplyResponseDTO mapResponse(
            Supply supply
    ) {
        ...
        return SupplyResponseDTO.builder()
                ...
                .meterNumber(
                        supply.getMeterNumber()
                )

                .internalReference(
                        supply.getInternalReference()
                )
                ...
                .customerName(
                        supply.getCustomer().getFullName()
                )

                .customerDocument(
                        supply.getCustomer().getDocumentNumber()
                )

                .propertyId(
                        supply.getProperty().getId()
                )

                .propertyAddress(
                        supply.getProperty().getAddress()
                )
                ...
                .activationDate(
                        supply.getActivationDate()
                )

                .build();
    }

    private SupplyDetailsDTO mapDetails(
            Supply supply
    ) {
        ...
        return SupplyDetailsDTO.builder()
                ...
                .propertyReference(
                        supply.getProperty().getReference()
                )

                .cadastralCode(
                        supply.getProperty().getCadastralCode()
                )
                ...
                .cutOffDate(
                        supply.getCutOffDate()
                )

                .reconnectionDate(
                        supply.getReconnectionDate()
                )

                .cutOffReason(
                        supply.getCutOffReason()
                )
